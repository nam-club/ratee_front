        ...
        >
          <template v-slot:activator="{ attrs }">
            <div>
              <InputSet
                type="textField"
                :caption="FORM_TITLE_TEXT"
                :captionLabel="FORM_CAPTION_REQUIRED_LABEL"
                :labelText="FORM_TITLE_LABEL"
                :textModel="title"
                @input="title = $event"
                :rules="titleRule"
              />
              <InputSet
                type="textsField"
                :caption="FORM_CHOICE_TEXT"
                :captionLabel="FORM_CAPTION_REQUIRED_LABEL"
                :textsModel="choices"
                @update:textsModel="choices = $event"
                :addText="FORM_ADD_CHOICE_TEXT"
                :rules="choiceRule"
              ...
              <InputSet
                type="chipBox"
                :caption="FORM_TAG_TEXT"
                :labelText="FORM_TAG_LABEL"
                :chipsModel="tags"
              ...
              <InputSet
                type="switchButton"
                :labelText="FORM_COMMENT_LABEL"
                :checkModel="enableComment"
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const errFlg = computed(() => {
      ...
      hasDuplicateChoices.value =
        new Set(nonEmptyChoices).size !== nonEmptyChoices.length;

      return (
        !title.value ||
        title.value.length < TITLE_MIN_LENGTH ||
        title.value.length > TITLE_MAX_LENGTH ||
        !choices.value ||
        choices.value.length < CHOICES_MIN_LENGTH ||
        choices.value.length > CHOICES_MAX_LENGTH ||
        choices.value.some(
          (choice) =>
            !choice ||
            choice === "" ||
            choice.length < CHOICE_MIN_LENGTH ||
    ...
    const titleRule = ref({
      ...
      textLength: (value: string) => {
        ...
        return (
          (length >= TITLE_MIN_LENGTH && length <= TITLE_MAX_LENGTH) ||
          TITLE_MIN_LENGTH +
            "~" +
            TITLE_MAX_LENGTH +
            "文字以内で入力してください。"
    ...
    const tagRule = ref({
      ...
      textLength: (value: string) => {
        ...
        return (
          ...
          TAG_MIN_LENGTH + "~" + TAG_MAX_LENGTH + "文字以内で入力してください。"
        );
      },
    });
    ...
    const onSubmit = async () => {
      ...
      try {
        ...
        await props.createQuestionnaire(
          title.value,
          choices.value,
          categoryId.value,
          tags.value,
